...
# redis 连接对象
REDIS_HOST = 'bellcat.redis.cache.chinacloudapi.cn'
REDIS_PORT = 6379
REDIS_DB = 0
REDIS_PASSWORD = "fs5VDicGuYqD86PAqsSxE41WpIB6miLGrMiNnPY8REM="
...
def flush_client(host, port=REDIS_PORT, db=REDIS_DB, password=REDIS_PASSWORD):
    global REDIS_HOST, REDIS_PORT, REDIS_DB
    REDIS_HOST = host
    REDIS_PORT = port
    REDIS_DB = db

STORAGE_PREFIX = 'GrabDoll'
...
class RedisBase(object):
    ...
    def get(cls, pkey):
        ...
        if cls.get_client().type(_storage_key) != cls.data_type:
            print cls.data_type, cls.storage_key, cls.get_client().type(_storage_key)
            return None
